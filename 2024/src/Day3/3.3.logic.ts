    ...
    for (let y = 0; y < tiles.length; y++) {
        for (let x = 0; x < tiles[y].length; x++) {
            ...
            if (tile.getDepth() === currentDepth) {
                if (tile.getNorth() === null || tile.getEast() === null || tile.getSouth() === null || tile.getWest() === null) {
                    continue;
                }

                // Keep digging if this tile is surrounded by tiles with the same depth
                if (tile.getNorth().getDepth() >= currentDepth &&
                    tile.getEast().getDepth() >= currentDepth &&
                    tile.getSouth().getDepth() >= currentDepth &&
                    tile.getWest().getDepth() >= currentDepth && 
                    tile.getNorth().getEast().getDepth() >= currentDepth &&
                    tile.getNorth().getWest().getDepth() >= currentDepth &&
                    tile.getSouth().getEast().getDepth() >= currentDepth &&
                    tile.getSouth().getWest().getDepth() >= currentDepth
                ) {
